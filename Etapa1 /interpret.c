...
void init() {
    s = new_stack(10); 
    printf("Máquina virtual baseada em pilha inicializada. Capacidade: 10\n");
}
...
void interpret(const char *source) {
    ...
    num_args_read = sscanf(source, "%19s %19s", op, arg_str);

    if (num_args_read == EOF || num_args_read == 0) { 
        return;
    }
    ...
    if (strcmp(op, "push") == 0) {
        ...
        value2 = stack_pop(s); 
        if (value1 == 0) {
            fprintf(stderr, "Erro: Divisão por zero! Não é possível dividir por 0.\n");
        ...
        if (s != NULL) {
            ...
            fprintf(stderr, "Erro: Pilha não inicializada para limpar.\n");
        }
    }
